...
- Assistant + Get Help Now = ONE compact floating group (user spec, Aug 11, 2026): a single bottom-right flex-col items-end container with an 8–10px gap, robot directly above a compact card (rounded-[22px], red→rose gradient, md:min-w-[280px], no full-width mobile bar, no arcade bevel). Never re-spread them with independent viewport coordinates. The IN-GAME HUD group follows the SAME 8–10px gap spec (Aug 12, 2026: its old gap-4 read as a weird hole; user ordered it matched).
- Robot bubble idle float (nyaya-float) REMOVED on user order (Aug 12, 2026, "floating animation remove kr do") — idle robot is static everywhere; do not re-add. Speaking bounce + story-narration ring stay; nyaya-float keyframes remain in index.css only for map-node-float.
