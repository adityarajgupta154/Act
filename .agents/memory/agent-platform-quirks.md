...
- Post-merge setup deliberately skips the drizzle db push (Aug 2026): nothing consumes @workspace/db and the SUPABASE_DATABASE_URL secret is not a valid postgres URL, so the push can only fail. **Why:** it broke a task merge. **How to apply:** if Supabase persistence ever becomes real, request a correct connection string first, then restore the push step.
- Workflow restarts can strand a portless zombie server (EADDRINUSE in logs, old node process survives on timers). Kill the stray server process, then restart the workflow.
