...
import { SpCompletionPanel, type SpRunStats } from './SpCompletionPanel';
...
/**
 * Shared festive outdoor scenery — sky, park strip, clouds, bunting,
 * balloon, confetti, CSS school + wooden sign. (No robot mascot here —
 * the floating Nyaya AI widget already owns the bottom-right corner;
 * a second big robot behind it read as a bug. Removed on user request.)
 * Used by BOTH full-screen chrome phases (intro + success) so the two
 * reference screens live in one visual world. Pure decoration (all
 * aria-hidden); interactive content renders above it.
 */
function SpSceneryLayers({ schoolSide = 'left' }: { schoolSide?: 'left' | 'right' }) {
  const t = useStrings();
  return (
    <>
      ...
      <div
        aria-hidden
        className={cn(
          'hidden lg:block absolute top-[15%] w-36 opacity-95',
          schoolSide === 'left' ? 'left-[4%]' : 'right-[4%]',
        )}
      >
...
export function SafePathGame({
  ...
  onExit: () => void;
  /** Fired the moment the goal is reached, with the run's final numbers —
   *  the zone landing card renders the SAME completion panel from them. */
  onRunStats?: (run: SpRunStats) => void;
}) {
  const t = useStrings();
  ...
  /** One onRunStats report per run — goal can't double-fire upstream. */
  const goalReportedRef = useRef(false);
    ...
    (dir: SpDir) => {
      ...
      switch (event.type) {
        ...
        case 'goal': {
          chime('done');
          addPop('+200', s.pos);
          const secs = Math.round((Date.now() - runStartRef.current) / 1000);
          setElapsedSec(secs);
          // Hand the finished run's numbers upstream right away (goal =
          // run finished) so the zone landing card can render the SAME
          // completion panel even if the player exits before the quiz.
          if (!goalReportedRef.current) {
            goalReportedRef.current = true;
            onRunStats?.({
              score: s.score,
              safeDecisions: s.safeDecisions,
              wrongDecisions: s.wrongDecisions,
              elapsedSec: secs,
            });
          }
  ...
  if (phase === 'intro') {
    // Intro / start screen — recreation of the user's reference image
    // (Aug 2026). Same festive scenery + cream-panel language as the
    // completion screen; every element is real DOM bound to live level
    // data (badge, mission, how-to rows) — nothing is a screenshot.
    ...
    const colorSafeZone = (text: string) => {
      const parts = text.split(t.spSafeZoneWord);
      return parts.length === 1
        ...
        : parts.map((p, i) => (
            <React.Fragment key={i}>
              {p}
              {i < parts.length - 1 && (
                <span className="text-emerald-600 font-extrabold">{t.spSafeZoneWord}</span>
              )}
            </React.Fragment>
          ));
    };
    const introRows = [
      {
        chip: 'bg-emerald-100 border-emerald-200 text-emerald-600',
        icon: <Footprints className="w-6 h-6" aria-hidden />,
        text: t.spMission1,
      },
      {
        chip: 'bg-violet-100 border-violet-200 text-violet-600',
        icon: <MessageCircleQuestionMark className="w-6 h-6" aria-hidden />,
        text: t.spMission2,
      },
      {
        chip: 'bg-sky-100 border-sky-200 text-sky-600',
    ...
    return (
      <div className="fixed inset-0 z-30 overflow-hidden">
        <SpSceneryLayers schoolSide="right" />
        <button
          type="button"
          onClick={onExit}
          aria-label={t.chExitLabel}
        ...
        <div className="absolute inset-0 overflow-y-auto">
          <div className="min-h-full flex flex-col items-center justify-center px-3 py-8 md:py-10">
            {/* cream panel (shield emblem notched into its top edge) */}
            <div className="relative bg-[#FFFDF8] rounded-[2rem] md:rounded-[2.5rem] shadow-2xl border border-orange-100 max-w-lg w-full px-4 pt-9 pb-5 md:px-8 md:pt-11 md:pb-7 mt-7 text-center animate-in zoom-in-95 duration-300">
              <div className="absolute -top-7 left-1/2 -translate-x-1/2 sp-pop">
                <div className="w-14 h-14 bg-white rounded-2xl shadow-lg border border-violet-100 flex items-center justify-center">
                ...
                <Star className="absolute -left-5 top-2 w-3.5 h-3.5 text-amber-400 fill-amber-300" aria-hidden />
                <Star className="absolute -right-5 top-4 w-3 h-3 text-amber-400 fill-amber-300" aria-hidden />
              </div>

              {/* hero — the maze's own player sprite over soft bushes */}
              <div className="relative inline-block mt-1">
                <span aria-hidden className="absolute -bottom-1 -left-8 w-14 h-9 bg-emerald-200/90 rounded-full" />
                <span aria-hidden className="absolute -bottom-2 -right-9 w-16 h-10 bg-emerald-300/80 rounded-full" />
                <span aria-hidden className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-24 h-8 bg-emerald-100 rounded-full" />
                <Star className="absolute -left-10 top-3 w-4 h-4 text-amber-400 fill-amber-300 sp-float" aria-hidden />
                <Star className="absolute -right-10 top-6 w-3.5 h-3.5 text-violet-400 fill-violet-300 sp-float" style={{ animationDelay: '0.7s' }} aria-hidden />
                <img
                  src={SP_HERO_URL}
                  alt=""
                  aria-hidden
                  draggable={false}
                  className="relative h-28 md:h-36 object-contain mx-auto sp-float drop-shadow-md select-none"
                />
              </div>

              {/* multicolor outlined title + tagline */}
              <h1 className="font-display font-extrabold text-3xl md:text-[2.6rem] leading-tight mt-2 sp-title-stroke">
                {titleWords.map((w, i) => (
              ...
              <p className="font-display font-bold text-slate-800 text-sm md:text-base mt-1">{t.spTagline}</p>
              <p className="font-bold text-violet-700 text-[11px] md:text-xs mt-0.5">({t.spAwarenessTag})</p>

              {/* level badge */}
              <p className="inline-flex items-center gap-2 bg-gradient-to-b from-amber-300 to-amber-400 text-amber-900 font-display font-extrabold text-sm md:text-base px-5 py-1.5 rounded-full shadow-sm mt-3">
                <Star className="w-4 h-4 fill-amber-600 text-amber-600" aria-hidden />
                {t.spLevelLabel(level.n, SP_LEVELS.length)} · {tx(level.title)}
                <Star className="w-4 h-4 fill-amber-600 text-amber-600" aria-hidden />
              </p>

              {/* mission line */}
              <p className="text-sm md:text-lg text-sky-950 font-bold mt-3">{colorSafeZone(tx(level.mission))}</p>

              {/* how-to rows */}
              <ul className="text-left mt-2 border-t-2 border-dashed border-slate-200">
                {introRows.map((r, i) => (
                  <li
                    key={i}
                    className={cn(
                      'flex items-center gap-3.5 py-3 md:py-3.5',
                      i < introRows.length - 1 && 'border-b-2 border-dashed border-slate-200',
                    )}
                  >
                    <span className={cn('w-11 h-11 md:w-12 md:h-12 rounded-2xl border-2 flex items-center justify-center shrink-0 shadow-sm', r.chip)}>
                      {r.icon}
                    </span>
                    <span className="font-bold text-slate-800 text-sm md:text-base leading-snug">
                      {colorSafeZone(r.text)}
                    </span>
                  </li>
                ))}
              </ul>
              ...
              {/* controls hint */}
              <p className="inline-flex items-center gap-2 bg-sky-50 border border-sky-100 text-sky-600 font-bold text-xs md:text-sm px-4 py-2 rounded-xl mt-1">
                <Keyboard className="w-4 h-4 md:w-5 md:h-5" aria-hidden />
              ...
              <button
                type="button"
                onClick={startMaze}
                className="mt-4 w-full bg-gradient-to-b from-orange-400 to-orange-600 hover:from-orange-500 hover:to-orange-600 text-white font-display font-extrabold text-lg md:text-xl px-6 py-4 rounded-full shadow-lg shadow-orange-500/40 flex items-center justify-center gap-2.5 transition-all duration-200 hover:scale-[1.02] hover:shadow-xl active:scale-95 touch-manipulation focus-visible:ring-2 focus-visible:ring-orange-400 focus-visible:ring-offset-2"
              >
    ...
    return (
      <div className="fixed inset-0 z-30 overflow-hidden">
        {/* ---- layered illustrated backdrop (PRD §9.5 — gentle cartoon),
             shared with the intro screen ---- */}
        <SpSceneryLayers schoolSide="left" />
        ...
        <div className="absolute inset-0 overflow-y-auto">
          <div className="min-h-full flex flex-col items-center justify-center px-3 py-6 md:py-8 md:pb-14">
            ...
            {/* ---- central completion panel — the SAME SpCompletionPanel
                 the zone landing card renders (one source of truth) ---- */}
            <div className="relative bg-[#FFFDF6]/95 rounded-[2rem] md:rounded-[2.5rem] border border-orange-100 shadow-2xl w-full max-w-3xl mt-4 md:mt-5 px-3 py-4 md:px-7 md:py-6 animate-in zoom-in-95 duration-300">
              <SpCompletionPanel
                stats={{
                  score: session.score,
                  safeDecisions: session.safeDecisions,
                  wrongDecisions: session.wrongDecisions,
                  elapsedSec,
                }}
                onBackToMap={onExit}
