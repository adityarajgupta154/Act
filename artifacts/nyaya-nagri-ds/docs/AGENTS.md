# Nyaya Nagri Design System design system
...
workspace package (`@workspace/nyaya-nagri-ds`): other artifacts depend
...
- `docs/references/` — extraction evidence: `component-inventory.md` (family
  list, ranking, chunk plan, status), `components/<family>.md` build contracts,
  and `brand/` (verbatim user-supplied mascot/backdrop art + manifest; the app
  has NO image logo — the brand device is the two-tone text wordmark).

Nyaya Nagri specifics: the component library is IMPLEMENTATION-CODE-SEEDED from
`artifacts/nyaya-nagri` (stock scaffold components outside the inventory were
removed; families are added back chunk by chunk from app source). The display
face Fredoka is wired as `--font-display` through `scripts/theme-template.css`
(the DTCG schema has no display slot); components use the `font-display`
utility exactly like the app. Light tokens are source-mapped from the app's
`src/index.css`; DARK MODE IS DERIVED from the app's navy chrome (app ships
light-only).
...
`@workspace/nyaya-nagri-ds/components/...`. Never use a `@/` alias inside
...
for `@workspace/nyaya-nagri-ds` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/nyaya-nagri-ds` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/nyaya-nagri-ds/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/nyaya-nagri-ds/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/nyaya-nagri-ds/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/nyaya-nagri-ds/docs/migrating-expo.md`
