# Migrating Expo UI to Nyaya Nagri Design System

Read `artifacts/nyaya-nagri-ds/docs/AGENTS.md` and
`artifacts/nyaya-nagri-ds/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/nyaya-nagri-ds/lib/native-theme` and
  `@workspace/nyaya-nagri-ds/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/nyaya-nagri-ds/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/nyaya-nagri-ds` and retains only
