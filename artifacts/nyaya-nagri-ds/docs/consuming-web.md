# Consuming Nyaya Nagri Design System in web apps

Read `artifacts/nyaya-nagri-ds/docs/AGENTS.md` first. This guide covers
...
`artifacts/nyaya-nagri-ds/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/nyaya-nagri-ds/styles.css";
```
...
`node_modules/@workspace/nyaya-nagri-ds/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/nyaya-nagri-ds/components/ui/button";
import { cn } from "@workspace/nyaya-nagri-ds/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/nyaya-nagri-ds/hooks/use-toast";
```
...
`@workspace/nyaya-nagri-ds/components/ui/button`. Run the app's typecheck
