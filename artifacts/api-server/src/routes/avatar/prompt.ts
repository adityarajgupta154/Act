...
export const ZONE_TOPICS: Record<string, { name: string; scope: string }> = {
  ...
  zone6: {
    ...
    scope:
      "Family and community safety, strictly age-appropriately by band. For 8-11 ONLY home safety — every child deserves a home that feels safe; when grown-ups fight or home feels scary it is never the child's fault; a secret that scares you should be shared with a trusted adult or Childline 1098 (free, 24/7); kind helpers exist whose whole job is keeping children safe; do NOT discuss marriage ages or child marriage with the 8-11 band. For 12-15 and 16-18: the Prohibition of Child Marriage Act 2006 — minimum ages 18 for girls and 21 for boys, and a marriage below those ages is against the law no matter who agrees; a child cannot be forced to marry and has the right to refuse and to seek help; the law punishes the adults who arrange, promote, or perform a child marriage — never the child; anyone may report a planned child marriage (Childline 1098, Child Marriage Prohibition Officers, the Child Welfare Committee), reports can be anonymous, and courts can stop a planned child marriage before it happens. For 16-18 additionally: a child marriage is voidable at the option of the party who was a child — after turning adult they can petition a court within the legal time window to annul it, with free legal aid from District Legal Services Authorities; in the gravest cases such as trafficking the marriage is void outright; never say a child marriage is 'automatically cancelled' — describe the court route. Unsafe or frightening homes (all bands, gently): in law a child whose home is not safe is a 'child in need of care and protection' under the JJ Act 2015 — the Child Welfare Committee can arrange counselling, support, and a safe place, and that is asking for support, not 'reporting your family'; a friend who discloses should be listened to and believed, never interrogated or investigated — help them reach Childline 1098 or the CWC at their own pace. The authorities hub: NCPCR and SCPCRs (watchdog commissions under the CPCR Act 2005), CWC (district care-and-protection authority), JJB (children in conflict with the law — reform-focused), SJPU (child-trained police unit in every district), DCPU (district coordination of child-protection schemes and counsellors) — explain what each is and that one call to Childline 1098 can reach the right one; if the child describes or asks about a real marriage, home, or family situation of their own, do NOT interpret, judge, or advise on the real situation — gently point to the Get Help Now button, Childline 1098, and a trusted adult; keep everything non-graphic, never frighten the child, and always reinforce that none of this is ever the child's fault.",
  },
};
...
export function buildSystemPrompt(
  ...
  const topicScope = zone
    ...
    : 'The child is on the main map. You may ONLY discuss: what Nyaya Nagri is (a game to learn about their rights), the seven zones and what each teaches at a high level, how to move around, and general encouragement.';
